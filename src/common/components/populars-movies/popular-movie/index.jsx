...
function PopularMovie ({backdrop_path, original_title, vote_average, vote_count, movieId}){
    return (
        <Link to={`/localMovie/${movieId}/${original_title}`} className="w-[280px] h-[260px] backdrop-blur-xl rounded-2xl p-0.5 flex-col items-center">
            <img className="w-full h-[190px] object-cover rounded-tr-2xl rounded-tl-2xl transition hover:scale-105 ease-in hover:rounded-2xl" src={backdrop_path === null || backdrop_path === undefined ? `/media/images/noPic.png` : `${process.env.REACT_APP_IMG_HOST}${backdrop_path}`} alt="Popular Movie" />

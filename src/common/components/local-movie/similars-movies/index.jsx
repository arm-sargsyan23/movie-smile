...
export default function SimilarsMovies({movieName}){
    ...
    const similarMoviesData = useRequest(`https://api.themoviedb.org/3/search/movie?query=${movieName}&language=en-US&page=1`)
                ...
                <div className="w-[90%] mx-auto min-h-max text-gray-100 p-8 rounded-3xl flex flex-wrap justify-center gap-5">
                    {
                        similarsMovies.length > 1 ? similarsMovies.map(({poster_path, original_title, vote_average, vote_count, id}, i) => {
                            ...
                            <p style={{overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: "1", WebkitBoxOrient: "vertical"}} className="[font-size:_clamp(2em,3vw,10em)]">Not found similar movie for your request</p>
                        </div>
                    }
                </div>
            }
